...
const handleRegister = async () => {
  ...
  try {
    ...
    message.value = "Registration successful! Check your email for confirmation."
  } catch (error) {
...
<template>
  <div class="max-w-md mx-auto mt-10 p-6 bg-white rounded-lg shadow-xl border border-gray-200">
    ...
    <form @submit.prevent="handleRegister" class="space-y-4">
      ...
      <button :disabled="loading" type="submit" class="w-full bg-blue-600 text-white py-2 rounded-md hover:bg-blue-700 transition">
        {{ loading ? 'Creating...' : 'Sign Up' }}
      </button>
      <p v-if="message" class="mt-4 text-sm text-center text-blue-600 font-medium">{{ message }}</p>
    </form>
  </div>
</template>